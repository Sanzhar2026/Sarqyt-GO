  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
      <div className="relative h-56">
        <Image src={getImageByTitle()} alt={name} fill className="object-cover" />
        <button onClick={toggleFavorite} className="absolute top-2 right-2 bg-black/40 backdrop-blur-sm rounded-full p-1.5 z-10">
        ...
        <button onClick={() => setShowExpanded(!showExpanded)} className="absolute bottom-2 right-2 bg-black/40 backdrop-blur-sm rounded-full p-1 z-10">
          <svg className="w-3 h-3 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </button>
        {discount > 0 && <div className="absolute top-2 left-2 bg-red-500 text-white px-1.5 py-0.5 rounded-full text-[9px] font-bold">-{discount}%</div>}
      </div>
      
      <div className="p-1.5">
        <Link href={`/supplier/${id}`}>
          <p className="font-bold text-[#367666] text-[10px] hover:text-[#2a5a4d] transition mb-0.5">{supplierName}</p>
        </Link>
        <h3 className="font-semibold text-gray-800 text-[10px] mb-0.5 line-clamp-1">{name}</h3>
        <div className="mb-0.5">
          <div className="text-gray-400 text-[8px] line-clamp-1">📍 {address || 'Адрес не указан'} • {pickupStartTime && pickupEndTime ? `${pickupStartTime}-${pickupEndTime}` : 'Время не указано'}</div>
        </div>
        <div className="flex items-center justify-between mt-0.5 mb-0.5">
          <div className="flex items-center gap-0.5">
            <div className="flex items-center gap-0.5">{renderStars()}</div>
            {bagTotalReviews > 0 && <span className="text-[7px] text-gray-400">({bagTotalReviews})</span>}
          </div>
        </div>
        {showExpanded && (
          <div className="mt-0.5 mb-0.5 p-1 bg-gray-50 rounded-lg">
            <div className="text-gray-600 text-[8px]">📍 Полный адрес: {address || 'Адрес не указан'}</div>
            <div className="text-gray-500 text-[8px] mt-0.5">🕒 Время: {pickupStartTime && pickupEndTime ? `${pickupStartTime}-${pickupEndTime}` : 'Время не указано'}</div>
          </div>
        )}
        <div className="flex items-center justify-between mt-0.5 pt-0.5 border-t border-gray-100">
          <div>
            ...
            {originalPrice > price && <span className="text-gray-400 line-through text-[6px] ml-0.5">{formatPrice(originalPrice)}</span>}
          </div>
          <button onClick={addToCart} disabled={addingToCart} className="bg-[#367666] text-white px-2 py-0.5 rounded-full text-[7px] font-medium hover:bg-[#2a5a4d] disabled:opacity-50 transition">
