// app/components/OfferCard.tsx - ВРЕМЕННАЯ ВЕРСИЯ БЕЗ ПРОВЕРКИ АВТОРИЗАЦИИ
'use client';
...
export default function OfferCard({
  ...
  const addToCart = async () => {
    ...
    try {
      // Пробуем получить user_id из localStorage или создаем тестового
      ...
      console.log('🛒 Добавление, user_id:', userId);
    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow">
      <Link href={`/offers/${id}`}>
        <div className="relative h-48">
          <Image src={getImage()} alt={name} fill className="object-cover" />
          {discount > 0 && (
